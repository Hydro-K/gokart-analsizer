      ...
      {activeTab === 'gforce' && tele && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {tele.lateral_acc && (
              <>
                <MetricCard label="Peak Lateral G (L)" value={Math.abs(Math.min(...tele.lateral_acc)).toFixed(2)} unit="G" />
                <MetricCard label="Peak Lateral G (R)" value={Math.abs(Math.max(...tele.lateral_acc)).toFixed(2)} unit="G" />
              </>
            )}
            {tele.inline_acc && (
              <>
                <MetricCard label="Peak Accel G"  value={Math.max(...tele.inline_acc).toFixed(2)} unit="G" highlight />
                <MetricCard label="Peak Brake G"  value={Math.abs(Math.min(...tele.inline_acc)).toFixed(2)} unit="G" />
              </>
            )}
          </div>
          ...
          {tele.lateral_acc && (
            <div className="bg-surface border border-border rounded-lg p-4 h-52">
              <h3 className="text-xs text-gray-400 uppercase tracking-wider mb-2">Lateral G vs Time  <span className="text-gray-600 normal-case">(+ = right, − = left)</span></h3>
              <ResponsiveContainer width="100%" height="85%">
                <AreaChart data={tele.time.map((t, i) => ({ t: +t.toFixed(2), g: +(tele.lateral_acc![i]).toFixed(3) }))}
                  margin={{ top: 5, right: 10, bottom: 10, left: 0 }}>
                  <defs>
                    <linearGradient id="latPos" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#FFD700" stopOpacity={0.4} />
                      <stop offset="95%" stopColor="#FFD700" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="latNeg" x1="0" y1="1" x2="0" y2="0">
                      <stop offset="5%" stopColor="#FFD700" stopOpacity={0.4} />
                      <stop offset="95%" stopColor="#FFD700" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid stroke={CHART_STYLE.grid} strokeDasharray="3 3" />
                  <XAxis dataKey="t" stroke={CHART_STYLE.axis} tick={{ fontSize: 10 }} label={{ value: 'Time (s)', position: 'insideBottom', offset: -5, fill: '#6a8090', fontSize: 10 }} />
                  <YAxis stroke={CHART_STYLE.axis} tick={{ fontSize: 10 }} tickFormatter={v => `${v}G`} />
                  <ReferenceLine y={0} stroke="#444" strokeDasharray="4 4" />
                  <Tooltip contentStyle={CHART_STYLE.tooltip} formatter={(v: any) => [`${v} G`, 'Lateral']} labelFormatter={t => `t = ${t}s`} />
                  <Area type="monotone" dataKey="g" stroke="#FFD700" strokeWidth={1.5} fill="url(#latPos)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {tele.inline_acc && (
            <div className="bg-surface border border-border rounded-lg p-4 h-52">
              <h3 className="text-xs text-gray-400 uppercase tracking-wider mb-2">Inline G vs Time  <span className="text-gray-600 normal-case">(+ = acceleration, − = braking)</span></h3>
              <ResponsiveContainer width="100%" height="85%">
                <AreaChart data={tele.time.map((t, i) => ({ t: +t.toFixed(2), g: +(tele.inline_acc![i]).toFixed(3) }))}
                  margin={{ top: 5, right: 10, bottom: 10, left: 0 }}>
                  <CartesianGrid stroke={CHART_STYLE.grid} strokeDasharray="3 3" />
                  <XAxis dataKey="t" stroke={CHART_STYLE.axis} tick={{ fontSize: 10 }} label={{ value: 'Time (s)', position: 'insideBottom', offset: -5, fill: '#6a8090', fontSize: 10 }} />
                  <YAxis stroke={CHART_STYLE.axis} tick={{ fontSize: 10 }} tickFormatter={v => `${v}G`} />
                  <ReferenceLine y={0} stroke="#444" strokeDasharray="4 4" />
                  <Tooltip contentStyle={CHART_STYLE.tooltip} formatter={(v: any) => [`${v} G`, 'Inline']} labelFormatter={t => `t = ${t}s`} />
                  <Area type="monotone" dataKey="g" stroke="#00BFFF" strokeWidth={1.5}
                    fill="#00BFFF" fillOpacity={0.15} dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {tele.lateral_acc && tele.inline_acc && (() => {
            const ggData = tele.lateral_acc.map((lat, i) => ({
              lat: +lat.toFixed(3),
              inl: +(tele.inline_acc![i]).toFixed(3),
            }))
            const maxG = Math.max(
              ...tele.lateral_acc.map(Math.abs),
              ...tele.inline_acc.map(Math.abs),
              0.5
            )
            const domain: [number, number] = [-(maxG * 1.1), maxG * 1.1]
            return (
              <div className="bg-surface border border-border rounded-lg p-4">
                <h3 className="text-xs text-gray-400 uppercase tracking-wider mb-1">GG Diagram  <span className="text-gray-600 normal-case">(traction circle)</span></h3>
                <p className="text-xs text-gray-600 mb-3">X = lateral G · Y = inline G · outer edge = limit of grip</p>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={ggData} margin={{ top: 10, right: 10, bottom: 10, left: 0 }}>
                      <CartesianGrid stroke={CHART_STYLE.grid} strokeDasharray="3 3" />
                      <XAxis dataKey="lat" type="number" domain={domain} stroke={CHART_STYLE.axis} tick={{ fontSize: 9 }}
                        tickFormatter={v => `${v}G`} label={{ value: 'Lateral G', position: 'insideBottom', offset: -5, fill: '#6a8090', fontSize: 10 }} />
                      <YAxis dataKey="inl" type="number" domain={domain} stroke={CHART_STYLE.axis} tick={{ fontSize: 9 }}
                        tickFormatter={v => `${v}G`} label={{ value: 'Inline G', angle: -90, position: 'insideLeft', fill: '#6a8090', fontSize: 10 }} />
                      <ReferenceLine y={0} stroke="#333" />
                      <ReferenceLine x={0} stroke="#333" />
                      <Tooltip contentStyle={CHART_STYLE.tooltip}
                        formatter={(_v: any, _n: any, props: any) => [
                          `Lat ${props.payload.lat}G  /  Inl ${props.payload.inl}G`, ''
                        ]} />
                      <Line dataKey="inl" dot={{ r: 1.5, fill: '#00BFFF', strokeWidth: 0 }} stroke="none" isAnimationActive={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
        ...
        <div className="space-y-4">
          {!energy ? (
            ...
            <>
              ...
              {/* Battery remaining gauge */}
              {(() => {
                const PACK_KWH = 3.072  // LiTime 48V 60Ah = 3072 Wh
                const usedPct = Math.min((energy.net_kwh / PACK_KWH) * 100, 100)
                const remainPct = 100 - usedPct
                const remainKwh = PACK_KWH - energy.net_kwh
                const lapsRemaining = energy.net_kwh > 0 ? remainKwh / energy.net_kwh : 0
                const barColor = remainPct > 40 ? '#00FF88' : remainPct > 20 ? '#FFD700' : '#FF4444'
                return (
                  <div className="bg-surface border border-border rounded-lg p-4 space-y-3">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Battery State (vs 3.072 kWh pack)</h3>
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-400">Used this lap: <span className="font-mono text-white">{energy.net_kwh.toFixed(4)} kWh</span></span>
                        <span style={{ color: barColor }} className="font-bold font-mono">{remainPct.toFixed(1)}% remaining</span>
                      </div>
                      <div className="h-4 bg-bg rounded-full overflow-hidden border border-border">
                        <div className="h-full rounded-full transition-all" style={{ width: `${remainPct}%`, background: barColor }} />
                      </div>
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>{remainKwh.toFixed(3)} kWh left</span>
                        <span>~{lapsRemaining.toFixed(1)} more laps at this pace</span>
                      </div>
                    </div>
                  </div>
                )
